    ...
    <Dialog open={open} onOpenChange={() => {}}>
      ...
      >
        <DialogHeader className="shrink-0 border-b px-6 py-4 text-left">
          <DialogTitle>{t('auth.selectDealer')}</DialogTitle>
          <DialogDescription>{t('auth.selectDealerSubtitle')}</DialogDescription>
        </DialogHeader>
        ...
        <div className="shrink-0 border-b px-4 py-3">
          <div className="relative">
            ...
            <Input
              type="search"
              placeholder={t('auth.filterRoles')}
              value={query}
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            {t('auth.shownCount', { shown: filteredRoles.length, total: roles.length })}
          </p>
        </div>
