...
import { issues, dailyAttendance, agencies, getIssuesByAgency } from '@/lib/mock-data'
...
export default function TrendsPage() {
  ...
  const issueDistribution = useMemo(() => {
    ...
    const distribution: Record<IssueType, number> = { late_arrival: 0, early_departure: 0, missing_entry: 0, missing_exit: 0, missing_lunch_out: 0, missing_lunch_in: 0, extended_lunch: 0, no_punches: 0 }
    filteredIssues.forEach(issue => { distribution[issue.type]++ })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-3"><TrendingUp className="h-7 w-7 text-[#4CAF50]" />Punctuality Trends</h1>
          <p className="text-muted-foreground mt-1">Attendance pattern and trend analysis</p>
        </div>
        <div className="flex items-center gap-3">
          <Select value={selectedAgency} onValueChange={setSelectedAgency}>
            <SelectTrigger className="w-[180px] border-border"><SelectValue placeholder="Dealer" /></SelectTrigger>
            <SelectContent><SelectItem value="all">All Dealers</SelectItem>{agencies.slice(0, 20).map(a => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent>
          </Select>
          <Select value={period} onValueChange={(v) => setPeriod(v as '7' | '14' | '30')}>
            ...
            <SelectContent><SelectItem value="7">Last 7 days</SelectItem><SelectItem value="14">Last 14 days</SelectItem><SelectItem value="30">Last 30 days</SelectItem></SelectContent>
          </Select>
          <ExportButton data={trendData} filename="trends-report" title="Trends Report" />
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-card border-border"><CardContent className="pt-4 pb-4"><p className="text-xs text-muted-foreground">Period Total</p><p className="text-2xl font-bold text-foreground">{comparison.currentCount}</p><p className={`text-xs ${comparison.change > 0 ? 'text-red-500' : 'text-[#4CAF50]'}`}>{comparison.change > 0 ? '+' : ''}{comparison.change}% vs previous</p></CardContent></Card>
        <Card className="bg-card border-border"><CardContent className="pt-4 pb-4"><p className="text-xs text-muted-foreground">Daily Average</p><p className="text-2xl font-bold text-foreground">{stats.avgDaily}</p><p className="text-xs text-muted-foreground">issues/day</p></CardContent></Card>
        <Card className="bg-red-500/10 border-red-500/30"><CardContent className="pt-4 pb-4"><p className="text-xs text-red-500">Worst Day</p><p className="text-2xl font-bold text-red-500">{stats.maxDay?.totalIssues || 0}</p><p className="text-xs text-red-400">{stats.maxDay?.displayDate}</p></CardContent></Card>
        <Card className="bg-[#4CAF50]/10 border-[#4CAF50]/30"><CardContent className="pt-4 pb-4"><p className="text-xs text-[#4CAF50]">Best Day</p><p className="text-2xl font-bold text-[#4CAF50]">{stats.minDay?.totalIssues || 0}</p><p className="text-xs text-[#4CAF50]">{stats.minDay?.displayDate}</p></CardContent></Card>
      </div>

      <Card className="bg-card border-border">
        <CardHeader className="pb-2"><CardTitle className="text-lg font-medium text-foreground">Issue Evolution</CardTitle></CardHeader>
        <CardContent>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trendData}>
                ...
                <Legend />
                <Area type="monotone" dataKey="totalIssues" name="Total" stroke="#ef4444" fill="url(#colorIssues)" strokeWidth={2} />
                <Line type="monotone" dataKey="lateArrivals" name="Late Arrivals" stroke="#FF9800" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="missingPunches" name="Missing Punches" stroke="#2196F3" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </CardContent>
