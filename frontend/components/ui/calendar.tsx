...
function CalendarDayButton({
  ...
  return (
    ...
    >
      {children}
      {/* "Today" dot indicator — only when not selected/in-range */}
      {modifiers.today && !isInRange && (
        <span className="absolute bottom-1 left-1/2 -translate-x-1/2 size-[3px] rounded-full bg-primary" />
      )}
    </Button>
  )
}
