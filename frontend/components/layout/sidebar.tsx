...
function SidebarInner({
  ...
  /** Accordion: at most one parent group expanded (by href). */
  ...
  useEffect(() => {
    ...
    // Only re-sync when the route changes; navigation identity changes often.
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional: pathname drives accordion
  }, [pathname])
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-3">
        <div className="space-y-1">
          {navigation.map((item) => {
            ...
            const isActive = hasChildren ? childActive : pathname === item.href
            const isGroupOpen = expandedGroupHref === item.href
            ...
            // Icon rail: parent link + optional child icon links (no accordion labels)
              ...
              return (
                <div key={item.href} className="space-y-1">
                  ...
                  {childItems.map((child) => {
                    const thisChildActive = pathname === child.href
                      ...
                      <div className="ml-5 space-y-1 border-l border-[var(--sidebar-subnav-rule)] pl-3">
                        {childItems.map((child) => {
                          ...
                          return (
                            ...
                            >
                              <ChildIcon className="h-4 w-4 shrink-0" />
                              <span className="overflow-hidden whitespace-nowrap">{child.name}</span>
                            </Link>
                          )
                        })}
                      </div>
