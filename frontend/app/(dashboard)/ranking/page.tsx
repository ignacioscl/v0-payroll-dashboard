...
export default function RankingPage() {
  ...
  const exportData = filteredRankings.map(r => ({
    [t('common.employee')]: `${r.employee.firstName} ${r.employee.lastName}`,
    [t('dealer.label')]: r.agency.name,
    [t('mockRanking.exportScore')]: r.punctualityScore,
    [t('mockRanking.exportIssues')]: r.totalIssues,
    [t('mockRanking.exportDaysWorked')]: r.totalDaysWorked,
    [t('mockRanking.exportOvertimeHours')]: r.overtimeHours
  }))
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-3"><Trophy className="h-7 w-7 text-yellow-500" />{t('mockRanking.title')}</h1>
          <p className="text-muted-foreground mt-1">{t('mockRanking.subtitle')}</p>
        </div>
        <ExportButton data={exportData} filename="employee-ranking" title={t('mockRanking.exportTitle')} />
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-card border-border"><CardContent className="pt-4 pb-4"><div className="flex items-center justify-between"><div><p className="text-xs text-muted-foreground">{t('mockRanking.averageScore')}</p><p className="text-2xl font-bold text-foreground">{stats.avgScore}%</p></div><Star className="h-8 w-8 text-muted-foreground/50" /></div></CardContent></Card>
        <Card className="bg-[#4CAF50]/10 border-[#4CAF50]/30"><CardContent className="pt-4 pb-4"><div className="flex items-center justify-between"><div><p className="text-xs text-[#4CAF50]">{t('mockRanking.perfectScore')}</p><p className="text-2xl font-bold text-[#4CAF50]">{stats.perfectScore}</p></div><TrendingUp className="h-8 w-8 text-[#4CAF50]/50" /></div></CardContent></Card>
        <Card className="bg-red-500/10 border-red-500/30"><CardContent className="pt-4 pb-4"><div className="flex items-center justify-between"><div><p className="text-xs text-red-500">{t('mockRanking.criticalScore')}</p><p className="text-2xl font-bold text-red-500">{stats.criticalScore}</p></div><TrendingDown className="h-8 w-8 text-red-500/50" /></div></CardContent></Card>
        <Card className="bg-card border-border"><CardContent className="pt-4 pb-4"><div className="flex items-center justify-between"><div><p className="text-xs text-muted-foreground">{t('mockRanking.totalIssues')}</p><p className="text-2xl font-bold text-foreground">{stats.totalIssues}</p></div><AlertTriangle className="h-8 w-8 text-muted-foreground/50" /></div></CardContent></Card>
      </div>

      {topThree.length >= 3 && (
        <Card className="bg-card border-border">
          <CardHeader className="pb-2"><CardTitle className="text-lg font-medium text-foreground">{view === 'best' ? t('mockRanking.top3MostPunctual') : t('mockRanking.top3MostIssues')}</CardTitle></CardHeader>
          <CardContent>
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <Tabs defaultValue="best" onValueChange={(v) => { setView(v as 'best' | 'worst'); setPageIndex(0) }}>
          <TabsList><TabsTrigger value="best" className="gap-2"><TrendingUp className="h-4 w-4" />{t('mockRanking.mostPunctual')}</TabsTrigger><TabsTrigger value="worst" className="gap-2"><TrendingDown className="h-4 w-4" />{t('mockRanking.mostIssues')}</TabsTrigger></TabsList>
        </Tabs>
        <div className="flex items-center gap-3">
          <div className="relative w-64"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input placeholder={t('mockRanking.searchEmployee')} value={search} onChange={(e) => { setSearch(e.target.value); setPageIndex(0) }} className="pl-10 bg-background border-border" /></div>
          <Select value={selectedAgency} onValueChange={(v) => { setSelectedAgency(v); setPageIndex(0) }}><SelectTrigger className="w-[180px] border-border"><SelectValue placeholder={t('dealer.label')} /></SelectTrigger><SelectContent><SelectItem value="all">{t('dealer.all')}</SelectItem>{agencies.slice(0, 20).map(a => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent></Select>
        </div>
      </div>
      ...
      <Card className="bg-card border-border">
        <CardContent className="p-0">
          <table className="w-full">
            <thead><tr className="bg-table-header text-white"><th className="px-4 py-3 text-left text-xs font-medium w-16 rounded-tl-md">#</th><th className="px-4 py-3 text-left text-xs font-medium">{t('common.employee')}</th><th className="px-4 py-3 text-left text-xs font-medium">{t('dealer.label')}</th><th className="px-4 py-3 text-center text-xs font-medium">{t('mockRanking.score')}</th><th className="px-4 py-3 text-center text-xs font-medium">{t('mockRanking.issues')}</th><th className="px-4 py-3 text-center text-xs font-medium">{t('mockRanking.daysWorked')}</th><th className="px-4 py-3 text-center text-xs font-medium">{t('mockRanking.overtime')}</th><th className="px-4 py-3 text-left text-xs font-medium w-40 rounded-tr-md">{t('mockRanking.progress')}</th></tr></thead>
          ...
          <div className="flex items-center justify-between border-t border-border px-4 py-3">
            <p className="text-sm text-muted-foreground">{t('mockRanking.showingOf', { shown: paginatedRankings.length, total: filteredRankings.length })}</p>
